...
import { guideState, drawGuides } from './guides.js';
...
function syncVideoCSS() {
    const visible = state.showVideo && state.activeTab !== 'alphabet';
...
function render() {
    ...
    if (state.activeTab === 'alphabet') {
        ...
        if (state.showBBox) {
            guidesCtx.save();
            guidesCtx.strokeStyle = 'rgba(20, 70, 255, 0.4)';
            guidesCtx.lineWidth = 1;
            guidesCtx.setLineDash([4, 4]);
            guidesCtx.strokeRect(guW * 0.1, guH * 0.05, guW * 0.8, guH * 0.9);
