...
export class ShapeEditor {
    constructor(canvas) {
        ...
        /** Called just before a state mutation (for undo snapshots). */
    ...
    /**
     * Find an endpoint (first or last point) of any shape near the given
     * coordinates. Returns { shapeId, pointIndex, fromStart } or null.
     */
    _hitTestEndpoint(fx, fy, radius = 0.03) {
        for (const shape of this.shapes) {
            if (shape.type === SHAPE_TYPES.DOT) continue;
            if (shape.points.length < 1) continue;
            const first = shape.points[0];
            const last  = shape.points[shape.points.length - 1];
            if (Math.hypot(first.x - fx, first.y - fy) < radius) {
                return { shapeId: shape.id, pointIndex: 0, fromStart: true };
            }
            if (shape.points.length >= 2 && Math.hypot(last.x - fx, last.y - fy) < radius) {
                return { shapeId: shape.id, pointIndex: shape.points.length - 1, fromStart: false };
            }
        }
        return null;
    }
    ...
    _onMouseDown(e) {
        ...
        if (this._drawingMode) {
            // Check for close-loop (clicking the opposite endpoint of active shape)
            if (this.activeShapeId) {
                ...
                if (hit) {
                    ...
                    // Drag existing point of active shape
            ...
            if (node) {
                ...
                if (shape) {
                    if (this._extendFromStart) {
                        shape.points.unshift({ x: node.x, y: node.y });
                        this._shiftJoinFlags(shape.id, -1, 1);
                        ...
                        shape.points.push({ x: node.x, y: node.y });
            ...
            // Click on empty space → finish
            this.finishActiveShape();
            return;
        }

        // ── EDIT / ALWAYS-READY MODE ──

        // 1. Try dragging an existing point (middle points only — endpoints auto-extend)
        const pointHit = this._hitTestPoint(raw.x, raw.y);
        if (pointHit) {
            const shape = this._getShape(pointHit.shapeId);
            const isEndpoint = shape && shape.type !== SHAPE_TYPES.DOT &&
                (pointHit.pointIndex === 0 || pointHit.pointIndex === shape.points.length - 1);

            if (!isEndpoint) {
                // Middle point → drag
                this._dragging = { shapeId: pointHit.shapeId, pointIndex: pointHit.pointIndex };
                this.selectedShapeId = pointHit.shapeId;
        ...
        // 2. Check if clicking a node
        const node = this._findNode(raw.x, raw.y);
        if (node) {
            e.preventDefault();
            if (this.onBeforeMutate) this.onBeforeMutate();

            // 2a. Dot tool — place and auto-finish
            if (this.activeTool === SHAPE_TYPES.DOT) {
                const shape = createShape(SHAPE_TYPES.DOT, [{ x: node.x, y: node.y }],
            ...
            // 2b. Node has an endpoint on it → extend that shape
            const endpointHit = this._hitTestEndpoint(node.x, node.y, 0.03);
            if (endpointHit) {
                this.extendShape(endpointHit.shapeId, endpointHit.fromStart);
                return;
            }

            // 2c. Unoccupied node → start new shape
            this.selectedShapeId = null;
            const tangency = this.activeTool === SHAPE_TYPES.ARC ? this.tangencyStrength : null;
            const shape = createShape(this.activeTool, [{ x: node.x, y: node.y }],
        ...
        // 3. Try selecting a shape by its segment
    ...
    _onKeyDown(e) {
        if (e.key === 'Escape') {
            ...
            // In draw mode: remove last placed point (undo last click)
            if (this._drawingMode && this.activeShapeId) {
                const shape = this._getShape(this.activeShapeId);
                if (shape && shape.points.length > 0) {
                    if (this.onBeforeMutate) this.onBeforeMutate();
                    e.preventDefault();
                    const idx = this._extendFromStart ? 0 : shape.points.length - 1;
                    this.joinFlags.delete(`${shape.id}:${idx}`);
                    if (this.onPointDeleted) this.onPointDeleted(shape.id, idx);
                    shape.points.splice(idx, 1);
                    if (this._extendFromStart) this._shiftJoinFlags(shape.id, 0, -1);
                    // If no points left, discard shape and exit draw mode
            ...
            // In edit mode: delete the selected point
